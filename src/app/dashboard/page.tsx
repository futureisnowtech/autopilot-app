...
export default function Dashboard() {
          ...
          if (stoppedByUserRef.current) {
            ...
            setTimeout(() => {
              try {
                ...
                console.error('Rapid restart error:', err);
              }
            }, 100);
          }
        };
  ...
  return (
    <div className="space-y-12">
              ...
              <Card className="bg-green-500/5 border border-green-500/20 rounded-[32px] overflow-hidden p-8 shadow-2xl text-left relative">
                ...
                <div className="border-t border-white/5 pt-4 space-y-2">
                  ...
                  {showAiInsights && (
                    ...
                    >
                      {successTask.notes && (() => {
                        const lines = (successTask.notes || '').split('\n');
                        const originalLine = lines.find((l: string) => l.startsWith('📝 Original:'));
                        const original = originalLine ? originalLine.replace('📝 Original: ', '') : null;
                        const aiBullets = lines.filter((l: string) => l.startsWith('• '));
                        const contextLine = lines.find((l: string) => l.startsWith('📋 Context:'));
                        const context = contextLine ? contextLine.replace('📋 Context: ', '') : null;

                        return (
                          <div className="space-y-3">
                            {original && (
                              <div>
                                <span className="text-xs font-bold text-slate-600 block mb-1">What You Said:</span>
                                <p className="text-sm text-slate-300 italic bg-white/5 px-3 py-2 rounded-lg border border-white/5">"{original}"</p>
                              </div>
                            )}
                            {aiBullets.length > 0 && (
                              <div>
                                <span className="text-xs font-bold text-slate-600 block mb-1">🤖 AI Enhancements:</span>
                                <ul className="space-y-1">
                                  {aiBullets.map((b: string, i: number) => (
                                    <li key={i} className="text-sm text-indigo-300 flex items-start gap-2">
                                      <Sparkles className="w-3 h-3 mt-1 shrink-0 text-indigo-500" />
                                      <span>{b.replace('• ', '')}</span>
                                    </li>
                                  ))}
                                </ul>
                            ...
                            {context && (
                              <div>
                                <span className="text-xs font-bold text-slate-600 block mb-1">Context:</span>
      ...
      <AnimatePresence>
        {showAskModal && (
          <AskAboutCalendarModal onClose={() => setShowAskModal(false)} onTasksChanged={() => userId && fetchRecentTasks(userId)} />
        )}
      </AnimatePresence>
